...
class TestSelector::TestHelperTest < ActiveSupport::TestCase
  include TestSelector::TestHelper
  ...
  test 'right value for 1 argument' do
    assert_equal test_selector('/some/long/path.html.erb'), 'test-selector=__some_long_path_html_erb'
  end

  test 'finds the test-selector in a page' do
    html = '<html> <body><span test-selector="__some_long_path_html_erb"></span></body></html>'
    test_selector = test_selector('/some/long/path.html.erb')

    assert find_test_selector(html, test_selector) == '<span test-selector="__some_long_path_html_erb"></span>'
  end
  ...
  test 'finds all test-selectors in a page' do
    ...
    assert find_test_selector(html, test_selector) == '<span test-selector="__some_long_path_html_erb"></span><span test-selector="__some_long_path_html_erb"></span>'
  end
